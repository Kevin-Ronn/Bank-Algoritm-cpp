...
// Templated Queue-klass för inbäddade system
template <typename T, size_t Capacity>
class Queue {
private:
    T data[Capacity];    // Statisk array för att hålla köelement
    size_t front;        // Index för det första elementet
    size_t back;         // Index för nästa insättning
    size_t count;        // Räknare för antalet element i kön
...
public:
    ...
    bool enqueue(const T& item) {
        ...
        data[back] = item;
        back = (back + 1) % Capacity;  // Cirkulär indexering
        ++count;
        return true;
    }
    ...
    bool dequeue(T& item) {
        ...
        item = data[front];
        front = (front + 1) % Capacity;  // Cirkulär indexering
        --count;
        return true;
    }
